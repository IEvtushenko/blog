...
class Pagination
{
     ...
     * @param string $operation - не обязательный параметр, нужен для правильной генерации HTML ссылок
     * @return
     */
    public function __construct($total, $currentPage, $limit, $operation = 'main')
    {
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        //Костыль для того, чтобы файл работал и для общего модуля и для модуля категорий

        if ('category' == $this->operation) {
            $currentURI = rtrim($_SERVER['REQUEST_URI'], '/') . '&';//для GET запросов
            $currentURI = preg_replace('~[\s,&]page=[0-9]+~', '', $currentURI);
        } else {
            $currentURI = rtrim($_SERVER['REQUEST_URI'], '/') . '?';//для GET запросов
            $currentURI = preg_replace('~[\s,?]page=[0-9]+~', '', $currentURI);
        }
